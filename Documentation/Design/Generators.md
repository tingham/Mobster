...
A user paints one gesture and expects marks to appear repeated about an origin, at a chosen number of rotations. Axial symmetry is the same problem with two rotations, so it needs no separate treatment.
