...
struct FigureLimb {
    ...
    let upper: FigureFrustum
    let lower: FigureFrustum
    ...
    init(root: SIMD2<Float>, target: SIMD2<Float>, pole: SIMD2<Float>, upperLength: Float, lowerLength: Float, rootWidth: Float, jointWidth: Float, endWidth: Float) {
        let solve = FigureSolve(root: root, target: target, upper: upperLength, lower: lowerLength, pole: pole)
        upper = FigureFrustum(start: root, end: solve.joint, startWidth: rootWidth, endWidth: jointWidth)
        lower = FigureFrustum(start: solve.joint, end: solve.end, startWidth: jointWidth, endWidth: endWidth)
    }
