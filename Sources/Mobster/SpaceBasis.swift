...
struct SpaceBasis: Hashable, Sendable {
    ...
    /// Forty five degrees off level is one of rise to one of run, so holding the rise within the run keeps forward clear of the vertical it is crossed with.
    init(target: SIMD3<Float>, roll: Float) {
        let run = (target.x * target.x + target.z * target.z).squareRoot()
        let held = SIMD3<Float>(target.x, min(max(target.y, -run), run), target.z)
