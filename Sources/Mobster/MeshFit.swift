...
struct MeshFit {
    ...
    /// Locations the boundary is fitted to. Low, because a path is drawn over rather than measured against.
    let count: Int
    ...
    func path() -> [SIMD2<Float>] {
        ...
        let wanted = min(max(count, available.lowerBound), available.upperBound)
