...
struct MeshBoundaryTests {
    ...
    /// Two components side by side, the right of them meeting the background, so the boundary between them sits at column two and the silhouette at column three. Both run the height of the raster, which is what carries them past the floor a run is kept at.
    @Test func adjacentFragmentsThatDifferAreABoundaryAndOnesThatAgreeAreNot() {
        let seams = MeshBoundary(raster: raster(4, 4, [1, 1, 2, .max,
                                                      1, 1, 2, .max,
                                                      1, 1, 2, .max,
                                                      1, 1, 2, .max])).seams()
    ...
    @Test func aBoundaryStandsMidwayBetweenTheFragmentsThatDiffer() {
        ...
        #expect(seams[0].locations.sorted { $0.y < $1.y } == [SIMD2<Float>(2, 1), SIMD2<Float>(2, 2), SIMD2<Float>(2, 3)])
        #expect(seams[1].locations.sorted { $0.y < $1.y } == [SIMD2<Float>(3, 1), SIMD2<Float>(3, 2), SIMD2<Float>(3, 3)])
    }

    @Test func aBoundaryBetweenRowsStandsMidwayToo() {
        let seams = MeshBoundary(raster: raster(4, 2, [1, 1, 1, 1,
                                                      2, 2, 2, 2])).seams()

        #expect(seams.count == 1)
        #expect(seams[0].locations.sorted { $0.x < $1.x } == [SIMD2<Float>(1, 1), SIMD2<Float>(2, 1), SIMD2<Float>(3, 1)])
    }
    ...
    @Test func theSeamsComeOutInTheOrderOfTheirIdentities() {
        let seams = MeshBoundary(raster: raster(4, 4, [7, 3, 3, 5,
                                                      7, 3, 3, 5,
                                                      7, 3, 3, 5,
                                                      7, 3, 3, 5])).seams()
