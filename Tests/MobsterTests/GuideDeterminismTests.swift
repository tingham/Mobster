...
struct GuideDeterminismTests {
    ...
    private func run() -> [SIMD2<Float>] {
        let guide = Guide(adherence: Adherence(reach: 37))
        ...
        guide.update(field: field(at: 12.5), time: 1.25)
    ...
    @Test func steppingASegmentMatchesArrivingAtItInOneCall() {
        let stepped = Guide(adherence: Adherence(reach: 37))
        ...
        let single = Guide(adherence: Adherence(reach: 37))
