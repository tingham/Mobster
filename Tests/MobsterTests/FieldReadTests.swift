...
struct FieldReadTests {
    ...
    @Test func aReadIsTakenFromTheQueryAndNotFromTheCentreOfItsTexel() {
        let field = FieldFixture.field(paths: vertical, frame: frame, count: 100)
    ...
    @Test func aReadHoldsAgainstAFrameAwayFromTheOrigin() {
        ...
        let field = FieldFixture.field(paths: [[SIMD2<Float>(-10, 15), SIMD2<Float>(-10, 115)]], frame: placed, count: 100)
    ...
    @Test func aLocationOnThePathReportsWithinATexelOfZero() {
        let field = FieldFixture.field(paths: diagonal, frame: frame, count: 400)
